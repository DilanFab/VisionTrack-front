...
export default function Administradores() {
  ...
  const administradores = useMemo(
    () => usuarios.filter((u) => u.perfiles.some((p) => p.rol.rol_nombre === ROL_ADMINISTRADOR)),
    [usuarios]
  );

  const rolAdministradorId = useMemo(
    () => roles.find((r) => r.rol_nombre === ROL_ADMINISTRADOR)?.rol_id,
    [roles]
  );
  ...
  const handleNuevo = () => {
    setEditingId(null);
    setForm({ ...initialForm, rol_ids: rolAdministradorId ? [rolAdministradorId] : [] });
    setShowModal(true);
  };
  ...
  const handleEliminar = async (id: number) => {
    const confirmado = await confirmarEliminacion("El administrador se eliminará permanentemente.");
  ...
  const handleGuardar = async () => {
    ...
    if (!editingId && !form.usuario_contrasena.trim()) {
      mostrarError("La contraseña es obligatoria para un administrador nuevo.");
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        mostrarExito("Administrador actualizado correctamente.");
      } else {
    ...
    <div>
      ...
      <Modal show={showModal} onHide={handleCerrarModal} size="lg" scrollable>
        <Modal.Header closeButton>
          <Modal.Title>{editingId ? "Editar Administrador" : "Nuevo Administrador"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
